...
static void MSKAPI OsiMskStreamFuncLog(MSKuserhandle_t handle, MSKCONST char *str)
{
  if (handle) {
...
static void MSKAPI OsiMskStreamFuncWarning(MSKuserhandle_t handle, MSKCONST char *str)
{
  if (handle) {
...
static void MSKAPI OsiMskStreamFuncError(MSKuserhandle_t handle, MSKCONST char *str)
{
  if (handle) {
    ((CoinMessageHandler *)handle)->message(0, "MSK", str, ' ') << CoinMessageEol;
  } else {
...
checkMSKerror(int err, const char *mskfuncname, const char *osimethod)
{
  if (err != MSK_RES_OK) {
    ...
    std::cout << "ERROR: " << s << " (" << osimethod << " in OsiMskSolverInterface)" << std::endl;
...
MSKassert(int assertlevel, int test, const char *assertname, const char *osimethod)
{
  if (assertlevel > MSK_OSI_ASSERT_LEVEL && !test) {
    std::cout << "Assert: " << assertname << " (" << osimethod << " in OsiMskSolverInterface)" << std::endl;
...
void OsiMskSolverInterface::switchToLP(void)
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
void OsiMskSolverInterface::switchToMIP(void)
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
void OsiMskSolverInterface::resizeColType(int minsize)
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
void OsiMskSolverInterface::freeColType()
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
bool OsiMskSolverInterface::setIntParam(OsiIntParam key, int value)
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
bool OsiMskSolverInterface::setDblParam(OsiDblParam key, double value)
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
bool OsiMskSolverInterface::setStrParam(OsiStrParam key, const std::string &value)
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
bool OsiMskSolverInterface::getIntParam(OsiIntParam key, int &value) const
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
bool OsiMskSolverInterface::getDblParam(OsiDblParam key, double &value) const
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
bool OsiMskSolverInterface::getStrParam(OsiStrParam key, std::string &value) const
{
#if MSK_OSI_DEBUG_LEVEL > 3
  ...
  if (collower_ == NULL) {
    ...
    if (ncols > 0) {
      ...
      for (int k = 0; k < ncols; ++k) {
        if (dummy_tags[k] == MSK_BK_UP || dummy_tags[k] == MSK_BK_FR) {
          /* No lower */
          collower_[k] = -getInfinity();
        }

        if (dummy_tags[k] == MSK_BK_LO || dummy_tags[k] == MSK_BK_FR) {
          /* No upper */
...
void OsiMskSolverInterface::setRowLower(int i, double elementValue)
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
void OsiMskSolverInterface::setRowUpper(int i, double elementValue)
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
void OsiMskSolverInterface::setRowBounds(int elementIndex, double lower, double upper)
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
void OsiMskSolverInterface::setRowType(int i, char sense, double rightHandSide, double range)
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
void OsiMskSolverInterface::setRowSetTypes(const int *indexFirst,
  const int *indexLast,
...
void OsiMskSolverInterface::setContinuous(int index)
{
...
void OsiMskSolverInterface::setInteger(int index)
{
  int numcols = getNumCols();
...
void OsiMskSolverInterface::setContinuous(const int *indices, int len)
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
void OsiMskSolverInterface::setInteger(const int *indices, int len)
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
void OsiMskSolverInterface::addCol(const CoinPackedVectorBase &vec,
...
void OsiMskSolverInterface::deleteCols(const int num, const int *columnIndices)
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
void OsiMskSolverInterface::addRow(const CoinPackedVectorBase &vec,
...
void OsiMskSolverInterface::deleteRows(const int num, const int *rowIndices)
{
#if MSK_OSI_DEBUG_LEVEL > 3
...
void OsiMskSolverInterface::loadProblem(const CoinPackedMatrix &matrix,
  const double *collb,
  const double *colub,
  const double *obj,
  const double *rowlb,
...
void OsiMskSolverInterface::assignProblem(CoinPackedMatrix *&matrix,
  double *&collb,
  double *&colub,
  double *&obj,
  double *&rowlb,
...
void OsiMskSolverInterface::loadProblem(const int numcols,
  const int numrows,
  const int *start,
  const int *index,
  const double *value,
