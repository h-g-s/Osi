...
static void CPXPUBLIC OsiCpxMessageCallbackPrint(CoinMessageHandler *handler, const char *msg)
{
...
static void CPXPUBLIC OsiCpxMessageCallbackResultLog(void *handle, const char *msg)
{
  if (!*msg)
    return;
...
static void CPXPUBLIC OsiCpxMessageCallbackWarning(void *handle, const char *msg)
{
  if (handle) {
...
static void CPXPUBLIC OsiCpxMessageCallbackError(void *handle, const char *msg)
{
  if (handle) {
    OsiCpxMessageCallbackPrint((CoinMessageHandler *)handle, msg);
  } else {
...
void OsiCpxSolverInterface::switchToLP(void)
{
...
void OsiCpxSolverInterface::switchToMIP(void)
{
...
bool OsiCpxSolverInterface::setIntParam(OsiIntParam key, int value)
{
...
bool OsiCpxSolverInterface::setDblParam(OsiDblParam key, double value)
{
...
bool OsiCpxSolverInterface::setStrParam(OsiStrParam key, const std::string &value)
{
...
bool OsiCpxSolverInterface::getIntParam(OsiIntParam key, int &value) const
{
...
bool OsiCpxSolverInterface::getDblParam(OsiDblParam key, double &value) const
{
...
bool OsiCpxSolverInterface::getStrParam(OsiStrParam key, std::string &value) const
{
...
#if CPX_VERSION >= 800
  return (stat == 0 || stat == CPX_STAT_NUM_BEST || stat == CPX_STAT_ABORT_USER);
#else
  return (stat == 0 || stat == CPX_NUM_BEST_FEAS || stat == CPX_NUM_BEST_INFEAS || stat == CPX_ABORT_FEAS || stat == CPX_ABORT_INFEAS || stat == CPX_ABORT_CROSSOVER);
#endif
}
...
#if CPX_VERSION >= 800
  return ((probtypemip_ == false && (stat == CPX_STAT_OPTIMAL || stat == CPX_STAT_OPTIMAL_INFEAS))
    || (probtypemip_ == true && (stat == CPXMIP_OPTIMAL || stat == CPXMIP_OPTIMAL_TOL)));
#else
  return ((probtypemip_ == false && (stat == CPX_OPTIMAL || stat == CPX_OPTIMAL_INFEAS)) || (probtypemip_ == true && stat == CPXMIP_OPTIMAL));
#endif
}
...
bool OsiCpxSolverInterface::isProvenPrimalInfeasible() const
{
  ...
  return ((method == CPX_ALG_PRIMAL && stat == CPX_INFEASIBLE) || (method == CPX_ALG_DUAL && stat == CPX_UNBOUNDED) || stat == CPX_ABORT_PRIM_INFEAS || stat == CPX_ABORT_PRIM_DUAL_INFEAS);
#endif
}
...
bool OsiCpxSolverInterface::isProvenDualInfeasible() const
{
  ...
  return ((method == CPX_ALG_PRIMAL && stat == CPX_UNBOUNDED) || (method == CPX_ALG_DUAL && stat == CPX_INFEASIBLE) || stat == CPX_ABORT_DUAL_INFEAS || stat == CPX_ABORT_PRIM_DUAL_INFEAS);
#endif
}
  ...
  bool fullRay) const
{
  ...
  const CoinPackedVectorBase **cols = new const CoinPackedVectorBase *[2 * numrows];
  int newcols = 0;
  for (i = 0; i < numrows; ++i) {
    switch (sense[i]) {
    case 'L':
...
//-----------------------------------------------------------------------------
void OsiCpxSolverInterface::setRowLower(int i, double elementValue)
{
...
//-----------------------------------------------------------------------------
void OsiCpxSolverInterface::setRowUpper(int i, double elementValue)
{
...
//-----------------------------------------------------------------------------
void OsiCpxSolverInterface::setRowBounds(int elementIndex, double lower, double upper)
{
...
//-----------------------------------------------------------------------------
void OsiCpxSolverInterface::setRowType(int i, char sense, double rightHandSide,
  double range)
{
...
//#############################################################################
void OsiCpxSolverInterface::setContinuous(int index)
{
...
//-----------------------------------------------------------------------------
void OsiCpxSolverInterface::setInteger(int index)
{
...
//-----------------------------------------------------------------------------
void OsiCpxSolverInterface::setContinuous(const int *indices, int len)
{
...
//-----------------------------------------------------------------------------
void OsiCpxSolverInterface::setInteger(const int *indices, int len)
{
...
//-----------------------------------------------------------------------------
void OsiCpxSolverInterface::deleteCols(const int num, const int *columnIndices)
{
...
//-----------------------------------------------------------------------------
void OsiCpxSolverInterface::deleteRows(const int num, const int *rowIndices)
{
...
void OsiCpxSolverInterface::reset()
{
  int err;
  CPXCHANNELptr cpxresults;
  CPXCHANNELptr cpxwarning;
...
void OsiCpxSolverInterface::getBasisStatus(int *cstat, int *rstat) const
{
  ...
  for (int i = 0; i < ncol; i++) {
    ...
    case 3:
      ...
      break;
    default:
      printf("### ERROR: OsiCpxSolverInterface::getBasisStatus(): unknown column status: %d\n", cstat[i]);
      break;
    }
  }
  ...
  if (objsense == 1) {
    ...
    for (int i = 0; i < nrow; i++) {
      ...
      case 2:
        ...
        break;
      default:
        printf("### ERROR: OsiCpxSolverInterface::getBasisStatus(): unknown row status: %d\n", rstat[i]);
        break;
      }
    }
  } else { // objsense == -1
