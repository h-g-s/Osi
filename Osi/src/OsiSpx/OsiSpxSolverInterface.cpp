...
{
  std::cout << "ERROR: " << error << " (" << osimethod << " in OsiSpxSolverInterface)" << std::endl;
...
bool OsiSpxSolverInterface::setIntParam(OsiIntParam key, int value)
{
  bool retval = false;
  try {
    switch (key) {
...
bool OsiSpxSolverInterface::setDblParam(OsiDblParam key, double value)
{
  bool retval = false;
  try {
    switch (key) {
...
bool OsiSpxSolverInterface::getIntParam(OsiIntParam key, int &value) const
{
  bool retval = false;
  switch (key) {
  case OsiMaxNumIteration:
...
bool OsiSpxSolverInterface::getDblParam(OsiDblParam key, double &value) const
{
  bool retval = false;
  switch (key) {
  case OsiDualObjectiveLimit:
...
bool OsiSpxSolverInterface::getStrParam(OsiStrParam key, std::string &value) const
{
  switch (key) {
  case OsiSolverName:
    value = "SoPlex";
...
bool OsiSpxSolverInterface::isAbandoned() const
{
  ...
  return (stat == soplex::SPxSolver::SINGULAR || stat == soplex::SPxSolver::ERROR);
}
...
//-----------------------------------------------------------------------------
void OsiSpxSolverInterface::setRowType(int i, char sense, double rightHandSide,
  double range)
{
  double lower = 0.0;
  double upper = 0.0;
...
//#############################################################################
void OsiSpxSolverInterface::setContinuous(int index)
{
#if SOPLEX_VERSION >= 300
...
//-----------------------------------------------------------------------------
void OsiSpxSolverInterface::setInteger(int index)
{
#if SOPLEX_VERSION >= 300
  int pos = spxintvars_->pos(index);
#else
...
void OsiSpxSolverInterface::loadProblem(const int numcols, const int numrows,
