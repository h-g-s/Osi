...
extern void testSimplexAPI(const OsiSolverInterface *emptySi, const std::string &mpsDir);
}
...

{
  ...
  CoinPackedMatrix *mtx = new CoinPackedMatrix(true, &rowndxs[0], &colndxs[0], &coeffs[0], 14);
...
*/
  ...
  double zopt[] = { -42.0, -56 };
  CoinWarmStartBasis::Status goodStatus[] = { CoinWarmStartBasis::basic,
...
*/
    ...
    const CoinWarmStartBasis *wsb = dynamic_cast< CoinWarmStartBasis * >(si->getWarmStart());
  ...
  // Test apply cuts method
  {
    ...
    // Generate some cuts
    {
      ...
      // Generate valid col cut from thin air
      {
        ...
        for (c = 0; c < nc; c++)
          lb[c] = oslColLB[c] + 0.001;
        for (c = 0; c < nc; c++)
          ub[c] = oslColUB[c] - 0.001;
...
*/
  ...
  //  of row and column ordered.)
  {
    ...
    const CoinShallowPackedVector neededBySunCC = exmip1Si->getMatrixByRow()->getVector(1);
