...
int OsiObject::columnNumber() const
{
  return -1;
}
...
// Copy constructor
OsiTwoWayBranchingObject::OsiTwoWayBranchingObject(const OsiTwoWayBranchingObject &rhs)
...
*/
void OsiSimpleInteger::resetBounds(const OsiSolverInterface *solver)
{
  originalLower_ = solver->getColLower()[columnNumber_];
  originalUpper_ = solver->getColUpper()[columnNumber_];
}
// Redoes data when sequence numbers change
void OsiSimpleInteger::resetSequenceEtc(int numberColumns, const int *originalColumns)
{
  int i;
...
/* Column number if single column object -1 otherwise,
   ...
   Used by heuristics
*/
int OsiSimpleInteger::columnNumber() const
{
...
// Copy constructor
OsiIntegerBranchingObject::OsiIntegerBranchingObject(const OsiIntegerBranchingObject &rhs)
...
{
  const OsiSimpleInteger *obj = dynamic_cast< const OsiSimpleInteger * >(originalObject_);
  assert(obj);
...
void OsiIntegerBranchingObject::print(const OsiSolverInterface *solver)
{
  const OsiSimpleInteger *obj = dynamic_cast< const OsiSimpleInteger * >(originalObject_);
  assert(obj);
  int iColumn = obj->columnNumber();
...
// Redoes data when sequence numbers change
void OsiSOS::resetSequenceEtc(int numberColumns, const int *originalColumns)
{
  int n2 = 0;
...
// Copy constructor
OsiSOSBranchingObject::OsiSOSBranchingObject(const OsiSOSBranchingObject &rhs)
  : OsiTwoWayBranchingObject(rhs)
{
}
...
{
  const OsiSOS *set = dynamic_cast< const OsiSOS * >(originalObject_);
  assert(set);
  int way = (!branchIndex_) ? (2 * firstBranch_ - 1) : -(2 * firstBranch_ - 1);
  branchIndex_++;
...
// Print what would happen
void OsiSOSBranchingObject::print(const OsiSolverInterface *solver)
{
  const OsiSOS *set = dynamic_cast< const OsiSOS * >(originalObject_);
  assert(set);
  int way = (!branchIndex_) ? (2 * firstBranch_ - 1) : -(2 * firstBranch_ - 1);
...
*/
bool OsiLotsize::findRange(double value, double integerTolerance) const
{
 ...
 */
void OsiLotsize::floorCeiling(double &floorLotsize, double &ceilingLotsize, double value,
...
/* Column number if single column object -1 otherwise,
   ...
   Used by heuristics
*/
int OsiLotsize::columnNumber() const
{
  return columnNumber_;
}
 ...
 */
void OsiLotsize::resetBounds(const OsiSolverInterface *)
{
}
// Return "down" estimate
double
...
// Redoes data when sequence numbers change
void OsiLotsize::resetSequenceEtc(int numberColumns, const int *originalColumns)
{
  int i;
  for (i = 0; i < numberColumns; i++) {
    if (originalColumns[i] == columnNumber_)
...
// Copy constructor
OsiLotsizeBranchingObject::OsiLotsizeBranchingObject(const OsiLotsizeBranchingObject &rhs)
  : OsiTwoWayBranchingObject(rhs)
{
  down_[0] = rhs.down_[0];
  down_[1] = rhs.down_[1];
...
{
  const OsiLotsize *obj = dynamic_cast< const OsiLotsize * >(originalObject_);
  assert(obj);
  int iColumn = obj->columnNumber();
  int way = (!branchIndex_) ? (2 * firstBranch_ - 1) : -(2 * firstBranch_ - 1);
  if (way < 0) {
...
// Print
void OsiLotsizeBranchingObject::print(const OsiSolverInterface *solver)
{
  const OsiLotsize *obj = dynamic_cast< const OsiLotsize * >(originalObject_);
  assert(obj);
  int iColumn = obj->columnNumber();
