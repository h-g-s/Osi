...
void OsiTestSolverInterface::loadProblem(const CoinPackedMatrix &matrix,
  ...
  const double *rowlb, const double *rowub)
{
  ...
  if (matrix.isColOrdered()) {
    ...
    maxNumcols_ = static_cast< int >((1 + rowMatrix_.getExtraGap()) * rowMatrix_.getMinorDim());
...
void OsiTestSolverInterface::assignProblem(CoinPackedMatrix *&matrix,
...
void OsiTestSolverInterface::loadProblem(const int numcols, const int numrows,
...
int OsiTestSolverInterface::readMps(const char *filename, const char *extension)
{
  CoinMpsIO reader;
...
void OsiTestSolverInterface::writeMps(const char *filename,
