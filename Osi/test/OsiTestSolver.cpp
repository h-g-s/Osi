  ...
  const VOL_dvector &dual_ub)
{
  ...
  viol = 0;
  for (int i = 0; i < nc; ++i) {
    if ((v[i] > 0.0 && dual_ub[i] != 0.0) || (v[i] < 0.0 && dual_lb[i] != 0.0))
  ...
  const VOL_dvector &v)
{
  ...
  double viol = 0.0;
  for (i = 0; i < nc; ++i) {
    if ((v[i] > 0.0 && u[i] < dual_ub[i]) || (v[i] < 0.0 && u[i] > dual_lb[i])) {
      viol += v[i] * v[i];
    }
  }
...
void VOL_dual::compute_xrc(const VOL_dvector &xstar, const VOL_dvector &x,
...
int VOL_problem::solve(VOL_user_hooks &hooks, const bool use_preset_dual)
{
  ...
  for (iter_ = 1; iter_ <= parm.maxsgriters; ++iter_) { // main iteration
    ...
    const bool small_gap = VolAbs(dstar.lcost) < 0.0001 ? (gap < parm.gap_abs_precision) : ((gap < parm.gap_abs_precision) || (gap / VolAbs(dstar.lcost) < parm.gap_rel_precision));
