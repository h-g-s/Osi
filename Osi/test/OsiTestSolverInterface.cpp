...
void OsiTestSolverInterface::updateRowMatrix_() const
{
...
void OsiTestSolverInterface::updateColMatrix_() const
{
...
void OsiTestSolverInterface::checkData_() const
{
  int i;
...
void OsiTestSolverInterface::compute_rc_(const double *u, double *rc) const
{
...
bool OsiTestSolverInterface::test_zero_one_minusone_(const CoinPackedMatrix &m) const
{
...
void OsiTestSolverInterface::gutsOfDestructor_()
{
  rowMatrix_.clear();
  colMatrix_.clear();
  rowMatrixCurrent_ = true;
...
void OsiTestSolverInterface::rowRimAllocator_()
{
...
void OsiTestSolverInterface::colRimAllocator_()
{
...
void OsiTestSolverInterface::rowRimResize_(const int newSize)
{
...
void OsiTestSolverInterface::colRimResize_(const int newSize)
{
...
void OsiTestSolverInterface::convertBoundsToSenses_()
{
...
void OsiTestSolverInterface::convertSensesToBounds_()
{
  for (int i = getNumRows() - 1; i >= 0; --i) {
...
int OsiTestSolverInterface::compute_rc(const VOL_dvector &u, VOL_dvector &rc)
{
...
int OsiTestSolverInterface::solve_subproblem(const VOL_dvector &dual,
  ...
  double &pcost)
{
  ...
  lcost = (std::inner_product(rhs_, rhs_ + dsize, dual.v, 0.0) + std::inner_product(x.v, x.v + psize, rc.v, 0.0));
...
void OsiTestSolverInterface::initialSolve()
{
...
void OsiTestSolverInterface::resolve()
{
  int i;
...
bool OsiTestSolverInterface::setIntParam(OsiIntParam key, int value)
{
  switch (key) {
...
bool OsiTestSolverInterface::setDblParam(OsiDblParam key, double value)
{
  switch (key) {
...
bool OsiTestSolverInterface::setStrParam(OsiStrParam key, const std::string &value)
{
  bool retval = false;
  switch (key) {
  case OsiSolverName:
...
bool OsiTestSolverInterface::getIntParam(OsiIntParam key, int &value) const
{
  switch (key) {
  case OsiMaxNumIteration:
...
bool OsiTestSolverInterface::getDblParam(OsiDblParam key, double &value) const
{
  switch (key) {
  case OsiDualObjectiveLimit:
...
bool OsiTestSolverInterface::getStrParam(OsiStrParam key, std::string &value) const
{
  switch (key) {
  case OsiProbName:
...
bool OsiTestSolverInterface::isAbandoned() const
{
  // *THINK*: see the *THINK* in isProvenOptimal()
  return false;
}

bool OsiTestSolverInterface::isProvenOptimal() const
{
  ...
  // *THINK*: frequently...
  return (!isDualObjectiveLimitReached() && volprob_.iter() < volprob_.parm.maxsgriters);
}

bool OsiTestSolverInterface::isProvenPrimalInfeasible() const
{
...
bool OsiTestSolverInterface::isProvenDualInfeasible() const
{
  // LL: *FIXME* : at the moment the volume assumes dual feasibility...
  return false;
}

bool OsiTestSolverInterface::isPrimalObjectiveLimitReached() const
{
...
bool OsiTestSolverInterface::isDualObjectiveLimitReached() const
{
  return volprob_.parm.ubinit - volprob_.value < volprob_.parm.granularity;
}

bool OsiTestSolverInterface::isIterationLimitReached() const
{
...
void OsiTestSolverInterface::markHotStart()
{
  delete[] rowpriceHotStart_;
...
void OsiTestSolverInterface::solveFromHotStart()
{
...
void OsiTestSolverInterface::unmarkHotStart()
{
  delete[] rowpriceHotStart_;
...
bool OsiTestSolverInterface::isContinuous(int colNumber) const
{
  assert(continuous_ != NULL);
  if (continuous_[colNumber])
    return true;
  return false;
}
...
void OsiTestSolverInterface::setContinuous(int index)
{
  assert(continuous_ != NULL);
...
void OsiTestSolverInterface::setInteger(int index)
{
  assert(continuous_ != NULL);
  if (index < 0 || index > getNumCols()) {
    throw CoinError("Index out of bound.", "setContinuous",
...
void OsiTestSolverInterface::setContinuous(const int *indices, int len)
{
  assert(continuous_ != NULL);
  const int colnum = getNumCols();
  int i;
...
void OsiTestSolverInterface::setInteger(const int *indices, int len)
{
  assert(continuous_ != NULL);
  const int colnum = getNumCols();
  int i;
...
void OsiTestSolverInterface::setColSolution(const double *colsol)
{
...
void OsiTestSolverInterface::setRowPrice(const double *rowprice)
{
...
void OsiTestSolverInterface::addCol(const CoinPackedVectorBase &vec,
...
void OsiTestSolverInterface::addCols(const int numcols,
...
void OsiTestSolverInterface::deleteCols(const int num, const int *columnIndices)
{
  if (num > 0) {
    int *delPos = new int[num];
    CoinDisjointCopyN(columnIndices, num, delPos);
    std::sort(delPos, delPos + num);
    const int delNum = static_cast< int >(std::unique(delPos, delPos + num) - delPos);
...
void OsiTestSolverInterface::addRow(const CoinPackedVectorBase &vec,
...
void OsiTestSolverInterface::addRows(const int numrows,
...
void OsiTestSolverInterface::deleteRows(const int num, const int *rowIndices)
{
  if (num > 0) {
    int *delPos = new int[num];
...
{
  return copyData ? new OsiTestSolverInterface(*this) : new OsiTestSolverInterface();
}
...
void OsiTestSolverInterface::applyRowCut(const OsiRowCut &rc)
{
  const int rownum = getNumRows();
...
void OsiTestSolverInterface::applyColCut(const OsiColCut &cc)
{
  int i;
